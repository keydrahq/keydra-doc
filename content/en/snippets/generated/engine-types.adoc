...
| `TIKV`
| TiKV, a flat keyspace of bytes. Its keys map onto Keydra's better than Aerospike's do — they are strings of bytes, so the namespace tree and prefix globs work without translation, and a glob's literal prefix becomes the range scan TiKV actually offers. What it has none of is everything above that: no types, no server statistics, no command language, no Pub/Sub.
